...
<script>
    $('select#empleado').on('change',function(){
        var valor = $(this).val();
        if(valor=='0'){
             $.post("semana.php", function(htmlexterno){
...
using(id_grado) right join alimento using(clavealimento) where id_mes = $mes and id_empleado=$id order by fecha asc";
